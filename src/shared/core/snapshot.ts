...
export interface SnapshotEntry {
   ...
   * коллекция и отправщик правок. Уйдёт, когда оба перестанут это делать.
   */
  type?: 'ANIME'
  status: string | null
  ...
  progress: number
  /**
   * Прочитано томов. Такой же остаток от манги, как и вид: не хранится
   * и не читается, но объявлено необязательным ради тех же двух писателей.
   */
  volumes?: number
  /**
 ...
 *
 * Поля перечислены явно и вид с томами среди них не значатся: что не переписано
 * здесь, то на диск и не попадёт, даже если лежало в прочитанной записи.
 */
