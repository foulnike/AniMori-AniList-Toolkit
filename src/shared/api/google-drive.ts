...
// оба адреса Google.
//
// ПОЧЕМУ НЕ СКРЫТАЯ ПАПКА ПРИЛОЖЕНИЯ
// Скрытая папка (appDataFolder) была бы точнее всего: её не видно ни в Диске,
// ни чужим приложениям. Но её область drive.appdata Google не выдаёт входу
// с устройства — проверено живыми запросами: на email profile drive.appdata
// ответ «Invalid device flow scope», на email profile drive.file — код для
// устройства, хотя справочник обещает обе. Поэтому копия лежит обычным
// файлом в Моём диске, а просится drive.file — область на файлы, которые
// приложение создало само.
//
// Что из этого следует, без прикрас:
// 1. Копию видно человеку в Диске, и он вправе её переименовать или убрать.
//    Потеряв её, приложение просто создаст новую при следующей записи.
// 2. Чужих файлов приложение по-прежнему не видит вовсе: drive.file показывает
//    только своё, и перечисление ниже выдаёт единицы файлов, а не весь Диск.
// 3. Право привязано к клиенту: если в настройках сменить ключи Google
//    на другие, старая копия станет невидимой и останется лежать в Диске
//    сиротой. Это не поломка, а свойство области, и прятать его нечестно.
//
// ПОЧЕМУ ФАЙЛ ИЩЕТСЯ ПЕРЕБОРОМ, А НЕ УСЛОВИЕМ ПОИСКА ПО ИМЕНИ
...
// узнать по имени. Условие поиска по имени потребовало бы прятать апострофы,
// а видно отсюда всё равно только своё и в единицах штук. Поэтому своё
// просто перечисляется, а имя сверяется уже здесь: короче и без возни
// с кавычками. В запросе стоит одно условие — без корзины: убранная человеком
// копия лежит в корзине ещё месяц и иначе сошла бы за живую.
//
...
/**
 * Где заводится копия: корень Моего диска. Своей папки мы не заводим
 * нарочно: это лишний запрос и лишний номер, который надо где-то помнить,
 * ради одного файла. Имя файла говорит само за себя, а перенести его
 * куда угодно человек может сам: на право приложения на свой файл
 * перемещение не влияет.
 */
export const DRIVE_APP_ROOT = 'root'
...
/** Сколько файлов спрашивать за раз: своих там единицы. */
const PAGE_SIZE = '100'

/** Что спрашивается у Диска: своё и живое, без корзины. */
const LIST_QUERY = 'trashed=false'
...
/** Перечисляет файлы приложения: чужие в этот ответ не попадают. */
async function listFiles(access: string, what: string): Promise<DriveResult<DriveEntry[]>> {
  ...
  try {
    ...
    Logger('WARN', 'Google Диск: файлы приложения не перечислены', e)
...
/**
 * Проверяет, что пропуск годен и Диск отвечает. Перечисление —
...
export async function checkAccess(access: string): Promise<DriveResult<true>> {
  ...
  Logger('API', `Google Диск доступен, своих файлов ${list.value.length}`)
 ...
 * остаются теми же. Иначе в Диске развелись бы двойники с одним именем —
 * Диск это разрешает, а человек потом не поймёт, какая копия его.
 *
 * Место указывается только при создании и только как начальное: если человек
 * унёс копию в свою папку, обратно в корень её никто не тащит.
 */
export async function uploadText(
  ...
  // Новому файлу нужны имя и место, и они едут отдельной частью рядом
