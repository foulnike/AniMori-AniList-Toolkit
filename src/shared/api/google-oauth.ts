...
//
// ОБЛАСТЬ НА СВОИ ФАЙЛЫ, С ОПОЗНАНИЕМ
// Из Диска просится одна drive.file — право на файлы, которые приложение
// создало само, и больше ничего: ни чужих файлов, ни перечисления Диска.
//
// Сначала здесь стояла drive.appdata — скрытая папка приложения, и она была бы
// точнее. Справочник Google обещает входу с устройства обе области Диска,
// но живые запросы говорят другое: на «email profile drive.appdata» приходит
// «Invalid device flow scope», на одинокую drive.appdata — то же самое,
// а на «email profile drive.file» — честный код для устройства. Проверено
// четырьмя запросами подряд одним клиентом «Телевизоры и устройства
// с ограниченным вводом» с включённым Диском в том же проекте. Спорить
// с этим нечем, поэтому копия переехала из скрытой папки в обычный файл —
// подробности и цена переезда в шапке api/google-drive.ts.
//
// Рядом с областью Диска приходится ставить email и profile: к области любого
// API вход с устройства просит добавлять опознание человека. Это не сбор
// сведений — читать имя и почту всё равно нечем: опросник о человеке здесь
// не зовётся ни разу, пропуск уходит только на Диск. Если правило переменится,
// вторая попытка ниже спросит одну область Диска без опознания.
//
...
/**
 * Область на свои файлы в Диске. Публичная: её показывают человеку
...
/**
 * Что просится у входа с устройства, по порядку. Сначала область Диска вместе
 * с опознанием — так требует поток, — а если и это не принято, одна область
 * Диска: вдруг правило переменится.
 */
...
const SCOPE_HINT =
  ...
  'в том же проекте, а drive.file стоит в перечне областей на экране согласия'
