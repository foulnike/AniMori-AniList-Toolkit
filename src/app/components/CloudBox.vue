...
//    кнопки со значком и подписью направления. Значки общепринятые — лоток
//    со стрелкой наружу и внутрь; стрелки текстом (↑ и ↓) читались
//    указателем сортировки, а не выгрузкой и загрузкой;
...
// а нужны один раз — когда что-то не сошлось. Сама «i» стоит в строке
// площадки, сразу за названием: вопрос «где взять пропуск» возникает у этой
// строки, а не у заголовка панели.
//
...
<template>
  <div class="am-panel am-box">
    ...
    <template v-if="cloudPlace === 'yandex'">
      <!-- То, что делают каждый день: две крупные кнопки со значком
           и подписью направления. Значок общепринятый — лоток со стрелкой
           наружу и внутрь: так рисуют выгрузку и загрузку везде, и читается
           он до подписи. Направление всё равно подписано словами: «сохранить»
           и «забрать» на слух отличаются хуже, чем на вид. -->
      <div class="am-duo">
        <button
